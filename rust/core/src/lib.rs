...
pub mod def {
    ...
    u SELECT_COURSE_SUBCLASS_URL = "xsxk/zzxkyzbjk_xkBcZyZzxkYzb.html?gnmkdm=N253512";
    u SELECT_COURSE_SUBCLASS_V2_URL = "xsxk/zzxkyzb_xkZyZzxkYzbZjxb.html?gnmkdm=N253512";
    u DISPLAY_SUBCLASS_URL = "xsxk/zzxkyzb_xkZyDisplayZzxkYzbZjxb.html?gnmkdm=N253512";
...
impl Client {
    ...
    pub fn new_with_base(mut backend: Url) -> Self {
        ...
        // 关键：确保 base 路径以 / 结尾，否则 join("xtgl/...") 会把最后一段当文件名解析
        if !backend.path().ends_with('/') {
            backend.set_path(&(backend.path().to_string() + "/"));
        }
    ...
    /// 标记 init 已完成
    pub fn mark_init_done(&mut self) {
