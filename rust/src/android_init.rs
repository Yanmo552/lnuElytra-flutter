...
//!
//! The Kotlin-side `MyPlugin` loads `librust_lib_lnu_elytra.so` in its
//! companion object's `init {}` block and calls
//! `init_android(applicationContext)` in `onAttachedToEngine`. This module:
//!
//! 1. Stores the `JavaVM` in `ndk_context` (for use by other crates).
//! 2. Creates global references to the app `Context` and its `ClassLoader`,
//!    stored in module-level statics.
//! 3. Passes [`NdkRuntime`] (which reads handles from the above statics) to
...
/// Logs via Android's `android.util.Log` so output appears in logcat.
/// Works even before flutter_rust_bridge's tracing subscriber is initialized.
...
/// Zero-sized [`rustls_platform_verifier::android::Runtime`] implementation
/// that reads handles from the statics above. The single static instance is passed to `init_with_runtime`.
struct NdkRuntime;
...
/// Called by the Kotlin-side `MyPlugin.init_android(applicationContext)`.
///
/// Stores the `JavaVM` and app `Context` in `ndk_context` (for other crates)
/// and in module-level statics (for [`NdkRuntime`]).
...
pub unsafe extern "C" fn Java_com_mcitem_lnu_1elytra_MyPlugin_init_1android(
    ...
    let _ = unowned_env.with_env(|env| -> Result<(), jni::errors::Error> {
        ...
        // ---- Module-level statics (for NdkRuntime) --------------------------
        ...
        // ---- Hand off to rustls-platform-verifier ----------------------------
